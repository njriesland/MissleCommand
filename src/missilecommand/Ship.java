		...
		if (isAlive()) {
			for (Vector2f pos : positionList) {
				if (isInvincible()) {
					...
					ship.render(g, view, nextSegment, angle);
				}
			}
...
/*		if (isAlive()) {
	...
	private Particle createRandomParticle() {
		Particle p = new Particle(new Vector2f(xx,yy));
                if(yy-- < 161) {
                   yy = 166;
                }
                if(xx++ > 0) {
		...
		p.setPosition(bulletPos);
                p.setPosition(new Vector2f(0,190));
